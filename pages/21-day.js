...
const TWENTY_ONE_DAY_STORAGE_KEY =
    'taeafi_21_day_challenge';
...
const TWENTY_ONE_DAY_TASKS = [
    {
        ...
        icon: 'fa-brain',
        description:
            'التقليل من الأشياء التي تشتت تركيزك وتضعف انضباطك.',
        tasks: [
            {
                id: 'pornography',
                title: 'لا إباحية',
...

    {
        ...
        icon: 'fa-sun',
        description:
            'ابدأ يومك بوعي قبل أن تبدأ المشتتات بالسيطرة على وقتك.',
        tasks: [
            {
                id: 'early_wakeup',
...

    {
        ...
        icon: 'fa-dumbbell',
        description:
            'ثوابت يومية تساعدك على بناء هوية أكثر انضباطًا.',
        tasks: [
            {
                id: 'exercise',
...

    {
        ...
        icon: 'fa-mobile-alt',
        description:
            'استرجع السيطرة على هاتفك بدل أن يتحكم الهاتف بوقتك.',
        tasks: [
            {
                id: 'no_phone_morning',
...
function twentyOneDayGetUser() {
    if (
        ...
        typeof StorageManager.getUser === 'function'
    ) {
...
function twentyOneDayDifferenceInDays(
    startDate,
    endDate
) {
    const start =
        twentyOneDayParseDate(startDate);

    const end =
        twentyOneDayParseDate(endDate);
...
function twentyOneDayLoad() {
    ...
    try {
        ...
        console.warn(
            '21-Day Challenge: failed to load data.',
            error
        );
    }
...
function twentyOneDaySave(data) {
    try {
        ...
        console.error(
            '21-Day Challenge: failed to save data.',
            error
        );
...
function twentyOneDayGetCurrentDay(data) {
    ...
    const elapsed =
        twentyOneDayDifferenceInDays(
            data.startDate,
            twentyOneDayTodayKey()
        );

    return twentyOneDayClamp(
        elapsed + 1,
        1,
        21
    );
...
function twentyOneDayGetDayData(
    data,
    dayNumber
) {
    if (!data.days) {
...
function twentyOneDayCountTasks() {
    return TWENTY_ONE_DAY_TASKS.reduce(
        (total, section) =>
...
function twentyOneDayGetCompletedTaskCount(
    dayData
) {
    if (
        !dayData ||
...
function twentyOneDayCalculateDayScore(
    dayData
) {
    const totalTasks =
        twentyOneDayCountTasks();

    const completedTasks =
        twentyOneDayGetCompletedTaskCount(
            dayData
        );
...
function twentyOneDayCalculateOverallScore(
    data
) {
    const dayEntries =
...
function twentyOneDayRefreshStatistics(data) {
    data.totalScore =
        twentyOneDayCalculateOverallScore(data);

    const level =
        twentyOneDayGetLevel(
            data.totalScore
        );
 ...
 */
function twentyOneDaySetProgress(
    selector,
    percentage
) {
    const root =
 ...
 * rebuilding the page.
 */
function twentyOneDayUpdateDashboard(
    data
) {
    const root =
        twentyOneDayGetRoot();
    ...
    const currentDay =
        twentyOneDayGetCurrentDay(data);

    const dayData =
        twentyOneDayGetDayData(
            data,
            currentDay
        );
    ...
    const headerDescription =
        root.querySelector(
            '[data-21-header-description]'
        );

    if (headerDescription) {
        const pronouns =
            twentyOneDayGetPronouns();
    ...
    /*
     * Update relapse action.
     */
    twentyOneDayUpdateRelapseAction(
        dayData
    );
 ...
 */
function twentyOneDayUpdateRelapseAction(
    dayData
) {
    const root =
        twentyOneDayGetRoot();
 ...
 */
function twentyOneDayUpdateHistory(
    data
) {
    const root =
        twentyOneDayGetRoot();
...
function twentyOneDaySmartRefresh() {
    ...
    const updated =
        twentyOneDayUpdateDashboard(data);

    if (updated) {
        twentyOneDayPulseElement(
            '[data-21-today-score]'
        );
...
function startTwentyOneDayChallenge() {
    ...
    const challenge =
        twentyOneDayCreateChallenge();

    twentyOneDaySave(challenge);

    showToast(
        twentyOneDayGetGender() === 'female'
            ? 'بدأتِ تحدي 21 يوم'
            : 'بدأتَ تحدي 21 يوم'
    );
...
function toggleTwentyOneDayTask(
    dayNumber,
    taskId
) {
    const data =
    ...
    if (
        Number(dayNumber) !== currentDay
    ) {
        showToast(
            'يمكنك تعديل مهام اليوم الحالي فقط'
        );
...
function recordTwentyOneDayRelapse() {
    ...
    dayData.relapseCount =
        Number(dayData.relapseCount || 0) + 1;

    data.totalRelapses =
        Number(data.totalRelapses || 0) + 1;
    ...
    showToast(
        'تم تسجيل الانتكاسة وتحديث التقييم'
    );
}
...
function undoTwentyOneDayRelapse() {
    ...
    const confirmed =
        confirm(
            'هل تريد إلغاء تسجيل الانتكاسة لهذا اليوم؟'
        );
...
function renderTwentyOneDayTask(
    dayNumber,
    task,
    dayData
) {
...
function renderTwentyOneDaySection(
    dayNumber,
    section,
    dayData
) {
        ...
        <div class="animate-fade-in twenty-one-page">
            ...
            <div class="twenty-one-chapters">
                ...
                <div class="twenty-one-chapter">
                    ...
                    <span class="twenty-one-chapter-number">
                        1
                    </span>
                ...
                <div class="twenty-one-chapter">
                    ...
                    <div>
                        ...
                        <p>
                            ثوابت يومية تحافظ عليها مهما كان يومك.
                        </p>
    ...
    mainContent.innerHTML = `
        <div class="animate-fade-in twenty-one-page">
            ...
            <div class="twenty-one-today-card">
                ...
                ${TWENTY_ONE_DAY_TASKS.map(
                    section =>
                        renderTwentyOneDaySection(
                            currentDay,
                            section,
                            dayData
                        )
...
function updateTwentyOneDayProgressUI(data, currentDay) {
    ...
    const remainingDays =
        Math.max(21 - currentDay, 0);

    const progressValue =
        document.querySelector('.twenty-one-main-progress strong');
    ...
    const progressBar =
        document.querySelector(
            '.twenty-one-main-progress .twenty-one-progress-bar span'
        );
    ...
    const meta =
        document.querySelectorAll(
            '.twenty-one-progress-meta span'
        );

    if (meta.length >= 2) {
        meta[0].textContent =
            `اليوم ${twentyOneDayFormatNumber(currentDay)} من ٢١`;

        meta[1].textContent =
            `${twentyOneDayFormatNumber(remainingDays)} يوم متبقي`;
    }
}
...
function updateTwentyOneDayStatsUI(data) {
    ...
    const level =
        twentyOneDayGetLevel(overallScore);
    ...
    const levelIcon =
        stats[1].querySelector('.twenty-one-stat-icon i');
    ...
    // Relapses
    const relapseElement =
        stats[2].querySelector('strong');

    if (relapseElement) {
        relapseElement.textContent =
            twentyOneDayFormatNumber(
                data.totalRelapses || 0
            );
    }
 ...
 */
function updateTwentyOneDayTodayUI(
    data,
    currentDay,
    dayData
) {
    ...
    const taskProgressContainer =
        document.querySelector(
            '.twenty-one-task-progress'
        );

    if (taskProgressContainer) {
        const strong =
            taskProgressContainer.querySelector('strong');

        if (strong) {
            strong.textContent =
                `${twentyOneDayFormatNumber(completedTasks)} / ${twentyOneDayFormatNumber(totalTasks)}`;
        }

        const progress =
            taskProgressContainer.querySelector(
                '.twenty-one-progress-bar span'
            );
 ...
 */
function updateTwentyOneDayTasksUI(
    data,
    currentDay,
    dayData
) {
    document
        .querySelectorAll('.twenty-one-task')
        .forEach(button => {
            const onclick =
            ...
            const match =
                onclick.match(
                    /toggleTwentyOneDayTask\(\s*\d+\s*,\s*['"]([^'"]+)['"]\s*\)/
                );
...
function updateTwentyOneDayActionsUI(dayData) {
    ...
    const existingRelapseButton =
        actions.querySelector(
            '.twenty-one-relapse-button'
        );

    const existingUndoButton =
        actions.querySelector(
            '[onclick*="undoTwentyOneDayRelapse"]'
        );
    ...
    if (dayData.relapse) {
        ...
        if (!existingRelapseButton) {
            ...
            relapseButton.className =
                'twenty-one-relapse-button';

            relapseButton.innerHTML = `
                <i class="fas fa-exclamation-triangle"></i>
                تسجيل انتكاسة
            `;

            relapseButton.addEventListener(
                'click',
                recordTwentyOneDayRelapse
            );
...
function resetTwentyOneDayChallenge() {
    ...
    try {
        ...
        console.error(
            'Failed to reset 21-Day Challenge:',
            error
        );
    }
...
window.renderTwentyOneDayPage =
    renderTwentyOneDayPage;

window.startTwentyOneDayChallenge =
    startTwentyOneDayChallenge;

window.confirmTwentyOneDayStart =
    confirmTwentyOneDayStart;

window.toggleTwentyOneDayTask =
    toggleTwentyOneDayTask;

window.recordTwentyOneDayRelapse =
    recordTwentyOneDayRelapse;

window.undoTwentyOneDayRelapse =
    undoTwentyOneDayRelapse;

window.completeTwentyOneDay =
    completeTwentyOneDay;

window.resetTwentyOneDayChallenge =
    resetTwentyOneDayChallenge;

window.twentyOneDaySmartRefresh =
    twentyOneDaySmartRefresh;